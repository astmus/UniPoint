...
namespace MissCore.DataAccess
{
	...
	public class BotUnitRequest<TUnit> : IUnitRequest<TUnit> where TUnit : BaseBotUnit
	{
		...
		{
			_unit = unit;
			_unitData = MetaData<TUnit>.With(unit);
		...
		{
			return $"{_unitData[nameof(BotUnit.Template)]}";
		}
