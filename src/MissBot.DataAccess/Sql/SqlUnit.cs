...
namespace MissBot.DataAccess.Sql
{
    ...
    public record SQL<TUnit>(string rawSql = default) : BotRequest where TUnit:class
    {
        ...
        public  SQLCommand Command
            => rawSql != null ? rawSql : global::MissBot.Abstractions.DataAccess.SqlUnit.Parse<TUnit>(global::MissBot.Abstractions.Unit<TUnit>.Sample);
    
        public record Query(FieldNamesSelector<TUnit> selector) : SQL<TUnit>
        {         
            public  SQLCommand Command
                => Abstractions.DataAccess.SqlUnit.Entities<TUnit>(selector).Command;
        }
