...
namespace BotService.Connection.Extensions
{
    public static class ConnectionExtensions
    {
        ...
        public static async Task SyncCommandsAsync(
            ...
            {
            var s = JsonConvert.SerializeObject(new Telegram.Bot.Requests.SetMyCommandsRequest(commands.Select(s => new BotCommand() { Command = s.CommandName, Description = s.Description })));
               await botClient.MakeRequestAsync(
                        request: new Telegram.Bot.Requests.SetMyCommandsRequest(commands.Select(s=> new BotCommand() { Command = s.CommandName, Description = s.Description }))
                        {
                            Scope = scope,
                            LanguageCode = languageCode
    },
